$(document).ready(function () {
  ...
  // Initialize extractedVariables based on the initial state of lastVariables
  let extractedVariables =
    typeof lastVariables === 'object' && lastVariables !== null
      ? { ...lastVariables }
      : {};

  // Removed updateExtractedVariablesFromSummary function as it's deprecated
  ...
  // Removed extractVariablesFromMessage function as it's deprecated
  // Gửi tin nhắn đến backend
  async function sendMessageToBackend(messageText) {
    ...
    try {
      ...
      const response = await fetch(
        'http://localhost:3000/appointment-bot-with-structured-output/chat',
        {
          method: 'POST',
          headers: {
  ...
  function sendMessage() {
    ...
    // Gửi tin nhắn đến backend (Backend now handles all variable extraction)
