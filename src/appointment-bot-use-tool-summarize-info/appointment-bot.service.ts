...
export class AppointmentBotService {
  ...
  private createPrompt(): ChatPromptTemplate {
    return ChatPromptTemplate.fromMessages([
      ...
      - Mỗi tool chỉ được dùng ĐÚNG 1 lần ngoại trừ summarize_info.
      ...
      Khi người dùng mô tả triệu chứng nhưng không biết nên khám khoa nào, hãy sử dụng find_department_by_symptoms để gợi ý các khoa phù hợp. Sau khi gợi ý khoa, phải lưu ID khoa đó vào biến trích xuất qua tool summarize_info.

      SAU MỖI LẦN NHẬN TIN NHẮN MỚI:
      1. LUÔN LUÔN phải chạy tool summarize_info để tóm tắt các thông tin đã thu thập được, có thể chạy trước hoặc sau 1 tool khác
      2. Kiểm tra kết quả tóm tắt để biết những thông tin nào còn thiếu
      3. Đặc biệt lưu ý: Đây là quy trình bắt buộc, không được bỏ qua vì bất cứ lý do gì. Công cụ này rất quan trọng để quản lý thông tin người dùng.
